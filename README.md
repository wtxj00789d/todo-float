...
> 重要：安装包安装完成后，请手动打开一次 Todo Float。应用会在这次普通启动时自动注册开机启动项；如果安装后从未打开过，Windows 开机时不会自动运行 Todo Float。

> 开机时如果今天不需要提醒，Todo Float 会安静退出，不显示窗口；只有需要提醒时，才会弹出今天的 todo list。所以开机后没有弹窗，通常表示今天没有需要弹出的待办，或今天已经处理过弹窗。
...
也可以点击输入区右上角的麦克风按钮，调用 Windows 自带语音输入。
...
换句话说，不需要提醒时应用不会显示任何窗口；需要提醒时才显示“今天要做”的待办列表。
...
这不是广告，也没有赞助关系。只是为了让这个小工具尽量低成本、容易跑起来。（后期应该会直接开放openAI Compatible URL吧，大概……）
